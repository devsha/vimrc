...
" use both cscope and ctag for 'ctrl-]', ':ta', and 'ctrl-t'
set cscopetag
...
function s:Cycle_csdb()
    ...
    let newcsdbpath = s:Find_in_parent("cscope/load.vim",s:windowdir(),$HOME)
"    echo "Found load.vim at: " . newcsdbpath
"    echo "Windowdir: " . s:windowdir()
    if newcsdbpath != "Nothing"
      ...
      if !cscope_connection(3, "out", b:csdbpath)
        ...
        set nocsverb
        exe "cd " . b:csdbpath 
        exe "source " . b:csdbpath . "/cscope/load.vim "
        set csverb
        let &csverb = save_csvb
      endif
      "
